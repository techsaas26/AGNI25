...
export const registerUser = async (req, res, next) => {
  try {
    ...
    const A_ID = await UserIdTracker.getNextId(); // generate unique insider ID
    ...
    logger.info(`User registered successfully: ${savedUser.username} (A_ID: ${savedUser.A_ID})`);

    const subject = "Registration Confirmation";
    const text = `Dear ${savedUser.firstName},\n\nThank you for registering! Your unique A_ID is: ${savedUser.A_ID} and username is: "${savedUser.username}".\n\nBest regards,\nEvent Team`;
...
export const loginUser = async (req, res, next) => {
  try {
    ...
    const tokenPayload = { A_ID: user.A_ID };
    const token = jwt.sign(tokenPayload, config.JWT_SECRET, { expiresIn: config.JWT_EXPIRY || "7d" });

    logger.info(`User logged in successfully: ${username} (A_ID: ${user.A_ID})`);
